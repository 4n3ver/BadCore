...
module Parser =
    ...
    let bind (fn: 'a -> Parser<'b>) (m: Parser<'a>) : Parser<'b> =
        let parser state =
            match runOnInput m state with
            | Ok { Parsed = parsed; State = state } -> runOnInput (fn parsed) state
            | Error err -> Error err
    ...
    // Functor
    let map (fn: 'a -> 'b) (m: Parser<'a>) : Parser<'b> =
        bind (fn >> unit) m <?> $"(map %s{m.Label})"


    // Applicative
    let apply (fm: Parser<'a -> 'b>) (m: Parser<'a>) : Parser<'b> =
        fm |> andThen <| m |> map (fun (fn, p) -> fn p)
    ...
    let ( *> ) x y = lift2 (Function.constant id) x y
