...
module Auth =
    ...
    module Form =
        ...
        let create (username: string) (password: string) (nonce: Nonce): Form =
            ...
            form |> List.map (Pair.map2 urlEscape) |> Form
    ...
    let getToken (Form form: Form): AsyncResult<Token, Gateway.Error> =
        ...
        let parseResponse (response: HttpResponseMessage) (body: string) =
            ...
            let lsidCookie =
                Http.Cookies.create response
                |> Http.Cookies.get "lsid"
                |> Option.get
        ...
        post (createUri "/login_web_app.cgi") (Http.Content.urlEncodedForm form)
        >>= getResponseBody
